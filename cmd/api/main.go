...
func main() {
	...
	// Only in dev or uat environments do we drop the "/openapi" prefix
	// from all SNAP routes (for easier testing); prod keeps it since
	// that's the path mandated by the SNAP/ASPI spec.
	snapBasePath := "/openapi/v1.0"
	if appEnv == "dev" || appEnv == "uat" {
		snapBasePath = "/v1.0"
	}

	// Keep the swagger UI/spec in sync with whichever base path is active.
	docs.SwaggerInfo.SwaggerTemplate = strings.ReplaceAll(docs.SwaggerInfo.SwaggerTemplate, "/openapi/v1.0/", snapBasePath+"/")
