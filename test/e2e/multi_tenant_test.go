...
// callAs signs with a specific vendor's credentials and headers.
...
func TestE2E_MultiVendor_EachVendorUsesItsOwnCredentials(t *testing.T) {
	...
	seedFixedBill(s, bca.config.PartnerID, "678901234567890300", "1000.00")
	seedFixedBill(s, legacy.config.PartnerID, "678901234567890301", "2000.00")

	// Each vendor's own credentials work.
	fromBCA := s.callAs(t, bca, paymentPath,
		paymentPayload(bca.config.PartnerID, "678901234567890300", "PAY-MV-1", "1000.00"))
	require.Equal(t, http.StatusOK, fromBCA.status, fromBCA.raw)

	fromLegacy := s.callAs(t, legacy, paymentPath,
		paymentPayload(legacy.config.PartnerID, "678901234567890301", "PAY-MV-2", "2000.00"))
...
func TestE2E_MultiVendor_CredentialsDoNotCrossOver(t *testing.T) {
	bca, legacy := vendorBCA(), vendorLegacy()
	s := newServer(t, bca, legacy)
	seedFixedBill(s, bca.config.PartnerID, "678901234567890302", "1000.00")
	...
	resp := s.call(t, paymentPath,
		paymentPayload(bca.config.PartnerID, "678901234567890302", "PAY-MV-3", "1000.00"),
...
func TestE2E_MultiVendor_PartnerAndChannelPinnedPerVendor(t *testing.T) {
	bca, legacy := vendorBCA(), vendorLegacy()
	s := newServer(t, bca, legacy)
	seedFixedBill(s, bca.config.PartnerID, "678901234567890303", "1000.00")
	...
	resp := s.call(t, paymentPath,
		paymentPayload(bca.config.PartnerID, "678901234567890303", "PAY-MV-4", "1000.00"),
...
func TestE2E_MultiVendor_BodyHashEncodingIsPerVendor(t *testing.T) {
	...
	bca, legacy := vendorBCA(), vendorLegacy()
	s := newServer(t, bca, legacy)
	seedNoBillAccount(s, legacy.config.PartnerID, "678901234567890304")

	// base64 against the base64 vendor: accepted.
	ok := s.callAs(t, legacy, paymentPath,
		paymentPayload(legacy.config.PartnerID, "678901234567890304", "PAY-ENC-OK", "5000.00"))
	...
	bad := s.callAs(t, legacy, paymentPath,
		paymentPayload(legacy.config.PartnerID, "678901234567890304", "PAY-ENC-BAD", "5000.00"),
...
func TestE2E_MultiVendor_FieldStrictnessIsPerVendor(t *testing.T) {
	...
	bca, legacy := vendorBCA(), vendorLegacy()
	s := newServer(t, bca, legacy)
	seedFixedBill(s, bca.config.PartnerID, "678901234567890305", "1000.00")
	seedFixedBill(s, legacy.config.PartnerID, "678901234567890306", "1000.00")
	...
	strict := s.callAs(t, bca, paymentPath, sparse(bca.config.PartnerID, "678901234567890305", "PAY-STRICT-1"))
	...
	lax := s.callAs(t, legacy, paymentPath, sparse(legacy.config.PartnerID, "678901234567890306", "PAY-LAX-1"))
...
func TestE2E_MultiMerchant_VAsAreIsolatedByPartnerServiceID(t *testing.T) {
	...
	seedFixedBill(s, "12345", "678901234567890310", "100000.00")
	seedFixedBill(s, "67890", "678901234567890311", "250000.00")

	merchantA := s.callAs(t, bca, paymentPath,
		paymentPayload("12345", "678901234567890310", "PAY-MM-1", "100000.00"),
		withPartnerID("12345"))
	require.Equal(t, http.StatusOK, merchantA.status, merchantA.raw)

	merchantB := s.callAs(t, bca, paymentPath,
		paymentPayload("67890", "678901234567890311", "PAY-MM-2", "250000.00"),
		withPartnerID("67890"))
	...
	// on B's bill, not A's.
	seedFixedBill(s, "67890", "678901234567890312", "250000.00")
	crossed := s.callAs(t, bca, paymentPath,
		paymentPayload("67890", "678901234567890312", "PAY-MM-3", "100000.00"),
		withPartnerID("67890"))
...
func TestE2E_MultiMerchant_UnknownVAUnderKnownPartnerRejected(t *testing.T) {
	...
	bca := vendorBCA()
	s := newServer(t, bca)
	seedFixedBill(s, bca.config.PartnerID, "678901234567890312", "1000.00")

	resp := s.callAs(t, bca, paymentPath,
		paymentPayload(bca.config.PartnerID, "678901234567890399", "PAY-MM-4", "1000.00"))
...
func TestE2E_MultiMerchant_CallbacksGoToTheOwningMerchant(t *testing.T) {
	...
	a := seedFixedBill(s, "12345", "678901234567890320", "100000.00")
	a.NotificationURL = "https://merchant-a.example/callback"
	b := seedFixedBill(s, "67890", "678901234567890321", "250000.00")
	b.NotificationURL = "https://merchant-b.example/callback"

	require.Equal(t, http.StatusOK, s.callAs(t, bca, paymentPath,
		paymentPayload("12345", "678901234567890320", "PAY-CB-1", "100000.00"),
		withPartnerID("12345")).status)
	require.Equal(t, http.StatusOK, s.callAs(t, bca, paymentPath,
		paymentPayload("67890", "678901234567890321", "PAY-CB-2", "250000.00"),
	...
	assert.Equal(t, "https://merchant-a.example/callback", urls["12345678901234567890320"])
	assert.Equal(t, "https://merchant-b.example/callback", urls["67890678901234567890321"])
}
...
func TestE2E_MultiVendor_CommaSeparatedPartnerAllowList(t *testing.T) {
	...
	seedFixedBill(s, "11111", "678901234567890330", "1000.00")

	accepted := s.call(t, paymentPath,
		paymentPayload("11111", "678901234567890330", "PAY-ALLOW-1", "1000.00"),
		withPartnerID("11111"))
	require.Equal(t, http.StatusOK, accepted.status, accepted.raw)

	rejected := s.call(t, paymentPath,
		paymentPayload("11111", "678901234567890330", "PAY-ALLOW-2", "1000.00"),
		withPartnerID("22222"))
