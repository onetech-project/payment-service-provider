...
func TestE2E_Negative_Auth(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			s := newServer(t)
			payload := inquiryPayload(testPartnerID, "678901234567890200", "INQ-NEG-1")
			if tc.path == paymentPath {
				payload = paymentPayload(testPartnerID, "678901234567890200", "PAY-NEG-1", "1000.00")
			} else if tc.path == statusPath {
				payload = statusPayload(testPartnerID, "678901234567890200", "INQ-NEG-1")
			}
...
func TestE2E_Negative_WrongBodyHashEncodingRejected(t *testing.T) {
	...
	s := newServer(t)
	seedFixedBill(s, testPartnerID, "678901234567890201", "1000.00")

	resp := s.call(t, paymentPath,
		paymentPayload(testPartnerID, "678901234567890201", "PAY-ENC-1", "1000.00"),
...
func TestE2E_SignatureCoversTheBody(t *testing.T) {
	...
	s := newServer(t)
	seedFixedBill(s, testPartnerID, "678901234567890202", "1000.00")

	original := mustJSON(t, paymentPayload(testPartnerID, "678901234567890202", "PAY-TAMPER-1", "1000.00"))
	...
	stringToSign := crypto.BuildStringToSign(http.MethodPost, paymentPath, "", bodyHash, timestamp)
...
func TestE2E_PrettyPrintedBodyStillVerifies(t *testing.T) {
	...
	s := newServer(t)
	seedFixedBill(s, testPartnerID, "678901234567890203", "1000.00")

	pretty := "{\n  \"partnerServiceId\": \"" + testPartnerID + "\",\n" +
		"  \"customerNo\": \"678901234567890203\",\n" +
		"  \"virtualAccountNo\": \"" + testPartnerID + "678901234567890203\",\n" +
...
func TestE2E_Negative_ExternalID(t *testing.T) {
	t.Run("missing", func(t *testing.T) {
		s := newServer(t)
		resp := s.call(t, paymentPath,
			paymentPayload(testPartnerID, "678901234567890210", "PAY-EXT-1", "1000.00"),
			withoutExternalID())
	...
	t.Run("longer than 36 characters", func(t *testing.T) {
		...
		resp := s.call(t, inquiryPath,
			inquiryPayload(testPartnerID, "678901234567890211", "INQ-EXT-1"),
	...
	t.Run("reused with a different payload is a Conflict, not a 422", func(t *testing.T) {
		s := newServer(t)
		seedNoBillAccount(s, testPartnerID, "678901234567890212")
...
func TestE2E_Negative_InquiryMandatoryFields(t *testing.T) {
	for _, field := range []string{"partnerServiceId", "customerNo", "virtualAccountNo", "inquiryRequestId"} {
		t.Run(field, func(t *testing.T) {
			s := newServer(t)
			payload := inquiryPayload(testPartnerID, "678901234567890220", "INQ-MAND-1")
			payload[field] = ""
...
func TestE2E_Negative_PaymentMandatoryFields(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.field, func(t *testing.T) {
			s := newServer(t)
			payload := paymentPayload(testPartnerID, "678901234567890221", "PAY-MAND-1", "1000.00")
			tc.mutate(payload)
...
func TestE2E_Payment_TrxIDStaysOptional(t *testing.T) {
	...
	// payments with a 400.
	s := newServer(t)
	seedFixedBill(s, testPartnerID, "678901234567890222", "1000.00")

	payload := paymentPayload(testPartnerID, "678901234567890222", "PAY-NOTRX-1", "1000.00")
...
func TestE2E_Negative_PaymentFieldFormats(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			s := newServer(t)
			payload := paymentPayload(testPartnerID, "678901234567890223", "PAY-FMT-1", "1000.00")
			tc.mutate(payload)
...
func TestE2E_Negative_UnregisteredVA(t *testing.T) {
	...
	inq := s.call(t, inquiryPath, inquiryPayload(testPartnerID, "678901234567890230", "INQ-404-1"))
	...
	// rejecting it.
	pay := s.call(t, paymentPath, paymentPayload(testPartnerID, "678901234567890230", "PAY-404-1", "1000.00"))
...
func TestE2E_Negative_ExpiredBill(t *testing.T) {
	s := newServer(t)
	rec := seedFixedBill(s, testPartnerID, "678901234567890231", "1000.00")
	past := time.Now().Add(-time.Hour)
	rec.ExpiredDate = &past

	inq := s.call(t, inquiryPath, inquiryPayload(testPartnerID, "678901234567890231", "INQ-EXPIRED-1"))
	assert.Equal(t, http.StatusNotFound, inq.status, inq.raw)
	assert.Equal(t, domain.CodeInquiryExpired, inq.code())

	pay := s.call(t, paymentPath, paymentPayload(testPartnerID, "678901234567890231", "PAY-EXPIRED-1", "1000.00"))
	assert.Equal(t, http.StatusNotFound, pay.status, pay.raw)
...
func TestE2E_Negative_StatusTransactionNotFound(t *testing.T) {
	...
	resp := s.call(t, statusPath, statusPayload(testPartnerID, "678901234567890232", "UNKNOWN-REQ"))
...
func TestE2E_EveryRejectionCarriesStatusAndReason(t *testing.T) {
	...
	cases := []struct {
		...
		{"inquiry not found", inquiryPath,
			inquiryPayload(testPartnerID, "678901234567890240", "INQ-ENV-1"), "inquiryStatus", "inquiryReason", nil},
		{"inquiry mandatory field", inquiryPath,
			map[string]any{"partnerServiceId": testPartnerID}, "inquiryStatus", "inquiryReason", nil},
		{"payment not found", paymentPath,
			paymentPayload(testPartnerID, "678901234567890240", "PAY-ENV-1", "1000.00"),
			"paymentFlagStatus", "paymentFlagReason", nil},
		{"payment mandatory field", paymentPath,
			map[string]any{"partnerServiceId": testPartnerID}, "paymentFlagStatus", "paymentFlagReason", nil},
		{"status not found", statusPath,
			statusPayload(testPartnerID, "678901234567890240", "ST-ENV-1"),
			"paymentFlagStatus", "paymentFlagReason", nil},
...
func TestE2E_AuthFailuresCarryBareBody(t *testing.T) {
	...
	resp := s.call(t, inquiryPath,
		inquiryPayload(testPartnerID, "678901234567890241", "INQ-AUTH-1"),
