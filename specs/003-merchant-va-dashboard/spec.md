...
- **FR-003**: System MUST receive and process payment notifications from banks via `POST /v1.0/transfer-va/payment` (SNAP service code 25) accepting `PaymentRequest` body. Required: `trxId`, `paymentRequestId`, `paidAmount` (`trxId` reclassified from optional to required — feature 004 Phase 7 — since it's the ASPI sample's mandatory Payment identifier, replacing `inquiryRequestId` in that role; `inquiryRequestId` is still accepted, optional). Optional: `virtualAccountName`, `virtualAccountEmail`, `virtualAccountPhone`, `channelCode`, `hashedSourceAccountNo`, `sourceBankCode`, `cumulativePaymentAmount`, `paidBills`, `totalAmount`, `trxDateTime`, `referenceNo`, `journalNum`, `paymentType`, `flagAdvise`, `subCompany`, `billDetails`, `freeTexts`, `additionalInfo`.
