...
**Storage**: PostgreSQL — new table `va_accounts`; existing `va_transactions`, `va_bill_details`, `va_payments`, `va_customer_no_sequences`, `master_va_type`, `master_partner_service_ids`, `va_notification_deliveries` unchanged. Migrations via `migrate/migrate` under `db/migrations/`, next sequence number `000013`.
...
```text
db/migrations/
├── 000013_create_va_accounts.up.sql      # NEW — table, constraints, backfill
└── 000013_create_va_accounts.down.sql    # NEW — DROP TABLE only
...
| # | Task | File |
|---|---|---|
| T-001 | Write `000013_create_va_accounts.up.sql`: table + `UNIQUE (virtual_account_no)` + `UNIQUE (partner_service_id, customer_no)` + status CHECK + partner index. | `db/migrations/` |
...
| T-003 | Write `000013_create_va_accounts.down.sql` (`DROP TABLE IF EXISTS va_accounts`). | `db/migrations/` |
