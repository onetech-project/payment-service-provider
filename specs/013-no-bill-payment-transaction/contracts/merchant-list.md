...
Rows backfilled by migration `000013` appear here identically to newly registered ones. VA numbers with no registration (unmanaged/legacy, no `va_type`) do **not** appear in this listing — they are reachable through the transaction listing below.
