...
- There is intentionally no per-vendor/channel enforcement toggle and no merchant-side opt-out — both checks are unconditional from the moment this ships, per spec Assumptions. Do not add a bypass/feature-flag "to be safe"; that was explicitly rejected during planning.
